...
async function loadSectionsCustomization(tab) {
    ...
    let sections = await browser.storage.local.get(query);
    ...
    for (const item of sections[`available_${courseId}`]) {
        ...
        toggle.addEventListener('change', async () => {
            const query = {};
            query[`hidden_${courseId}`] = [];
            let hiddenItems = (await browser.storage.local.get(query))[`hidden_${courseId}`];
...
async function loadNavCustomization(tab) {
    let navItems = await browser.storage.local.get({
        availableNav: [],
        hiddenNav: []
    });
    ...
    for (const item of navItems.availableNav) {
        ...
        toggle.addEventListener('change', async () => {
            let hiddenItems = (await browser.storage.local.get({ hiddenNav: [] })).hiddenNav;
            if (toggle.checked && hiddenItems.includes(item)) hiddenItems = hiddenItems.filter(el => el !== item);
            else if (!toggle.checked && !hiddenItems.includes(item)) hiddenItems.push(item);
            else return;
            await browser.storage.local.set({ hiddenNav: hiddenItems });
            browser.tabs.reload(tab.id);
        });
        switchEl.appendChild(toggle);
...
async function loadTodoCustomization(tab) {
    let setting = (await browser.storage.local.get({ todosHidden: false })).todosHidden;
    ...
    toggle.addEventListener('change', async () => {
        let todosHidden = (await browser.storage.local.get({ todosHidden: false })).todosHidden;
        if (toggle.checked && todosHidden) todosHidden = false;
        else if (!toggle.checked && !todosHidden) todosHidden = true;
        else return;
        await browser.storage.local.set({ todosHidden: todosHidden });
        browser.tabs.reload(tab.id);
    });
    switchEl.appendChild(toggle);
...
async function loadAccentCustomization(tab) {
    let setting = await browser.storage.local.get({ accent: '1F6199' });
    ...
    const updateAccentColor = async () => {
        ...
        if (/^[0-9A-F]{6}$/i.test(newAccent) && newAccent != setting.accent) {
            await browser.storage.local.set({ accent: newAccent });
            browser.tabs.reload(tab.id);
        }
    }
